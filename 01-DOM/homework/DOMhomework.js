...
createdBy.innerHTML += " Martin Alderete";
...
function buildToDo(todo, index) {
  ...
  // hace lo mismo que el de arriba: toDoShell.className = "toDoShell"
  let toDoText = document.createElement("samp")
  toDoText.innerHTML = todo.description
  toDoText.setAttribute("id",index)
  toDoText.addEventListener("click", completeToDo)
  // hace lo mismo que el de arriba: toDoText.id = index
...
function buildToDos(toDos) {
  // Tu código acá:
  return toDos.map(buildToDo) // me crea ides 0,1,2,3,...
  // return toDos.map(e => buildToDo(e, "nombreParaIdentificar" + i++)) 
  // me crea ids nombreParaIdentificar0, nombreParaIdentificar1, nombreParaIdentificar2, nombreParaIdentificar3
...
addButton.addEventListener("click",addToDo) // no se invoca con (), ya que el add event listener se usa implementando la funcion, no invocando una, es por eso que se la pasa sin ()
